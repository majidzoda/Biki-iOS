...
class MapViewController: UIViewController, GMSMapViewDelegate {
    ...
    func updateMainUI() {
        print("FindTemp  \(currentWeather.currentTemp)")
        print("FindWeather " + currentWeather.weatherType)
        
        
        let position = CLLocationCoordinate2D(latitude: 40.763194, longitude: -73.978891)
        let nyc = GMSMarker(position: position)
        nyc.title = "NYC"
        nyc.icon = UIImage(named: currentWeather.weatherType)
        nyc.map = googleMapView
